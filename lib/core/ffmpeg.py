...
def audio_video_merge(video_file, audio_file, m4sfilepath, muxFormat, fastStart, OutPutPath, poster="", audioName="", title="",
    ...
    command = f"-loglevel warning -i {video_file} -i {audio_file}"
    ...
    switcher = {
        "MP4": lambda: command + " " + (('-i \"' + poster + '\"') if poster else '') +
                       ...
                       (" -c:a copy -c:v copy -y " +
                       " " + OutPutPath + ".mp4"),
        "MKV": lambda: command + "  -c:a copy -c:v copy -y "+
                       " " + OutPutPath + ".mkv",
        "FLV": lambda: command + "  --c:a copy -c:v copy -y " + ('-bsf:a aac_adtstoasc' if UseAACFilter else '') +
                       " " + OutPutPath + ".flv",
        "TS": lambda: command + "  -c:a copy -c:v copy -y -f mpegts -bsf:v h264_mp4toannexb" +
                       " " + OutPutPath + ".ts",
        "VTT": lambda: command + "  -y " + OutPutPath + ".srt",
        "EAC3": lambda: command + " -c:a copy -c:v copy -y " + OutPutPath + outputname + ".eac3",
        "AAC": lambda: command + " -c:a copy -c:v copy -y " + OutPutPath + outputname + ".m4a",
        "AC3": lambda: command + "  -c:a copy -c:v copy -y " + OutPutPath + outputname + ".ac3",
    }
    ...
    if command_builder:
        command = command_builder()
        subprocess.call([ffmpeg_path] + command.split(), cwd=m4sfilepath)
